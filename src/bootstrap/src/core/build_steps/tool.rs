...
pub struct Panickiller {
    /// This should only ever be 0 or 2.
    /// We sometimes want to reference the "bootstrap" rustdoc, which is why this option is here.
    pub compiler: Compiler,
}

impl Step for Panickiller {
    type Output = PathBuf;
    const DEFAULT: bool = true;
    const ONLY_HOSTS: bool = true;

    fn should_run(run: ShouldRun<'_>) -> ShouldRun<'_> {
        run.path("src/tools/panickiller").path("src/librustdoc")
    }

    fn make_run(run: RunConfig<'_>) {
        run.builder.ensure(Panickiller {
    ...
    fn run(self, builder: &Builder<'_>) -> PathBuf {
        ...
        if target_compiler.stage == 0 {
            ...
            return builder.initial_rustc.with_file_name(exe("panickiller", target_compiler.host));
        }
        ...
        let tool_rustdoc = builder
            ...
            .join(exe("panickiller", target_compiler.host));
        ...
        if target_compiler.stage > 0 {
            ...
            let bin_rustdoc = bindir.join(exe("panickiller", target_compiler.host));
